...
class AddTripStageForm(ModelForm):  # ModelForm if it depends on a model, forms.Form otherwise
    ...
    story = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control'}), label=_("Récit"), required=False)
    distance = forms.IntegerField(widget=forms.NumberInput(attrs={'class': 'form-control'}), label=_("Distance"), required=False)
    duration = forms.IntegerField(widget=forms.NumberInput(attrs={'class': 'form-control'}), label=_("Durée"), required=False)
    email_validation = forms.BooleanField(widget=forms.CheckboxInput(attrs={'class': 'form-control'}), label=_("Recevoir une validation par e-mail ?"), required=False)
    email = forms.EmailField(widget=forms.EmailInput(attrs={'class': 'form-control'}), label=_("Adresse E-mail"), required=False)

    coordinates = forms.CharField(label=_("Cliquer sur le lieu de votre aventure."), max_length=200, required=True)
    massif = forms.ModelChoiceField(widget=forms.Select(attrs={'class': 'form-control'}), queryset=Massif.objects, empty_label="----------", label=_("Massif"), required=False)
    ...
    class Meta:
        model = TripStage
        fields = ['coordinates', 'stage_name', 'date', 'massif', 'type', 'country', 'trip_linked', 'story', 'distance', 'duration', 'email_validation', 'email']
